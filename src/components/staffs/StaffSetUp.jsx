...
import {
    ...
    AlertCircle, Loader2, ChevronRight, Check, Camera,
    HeartPulse, FileText, Users, XCircle,
} from "lucide-react";
        ...
        <div className="min-h-screen bg-[#F7F4EF] font-['DM_Sans'] text-[#0D1117]">
            ...
            <div className="sticky top-0 z-20 bg-white border-b border-black/[0.09] px-6 py-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                    <h1 className="font-['DM_Serif_Display'] text-[18px] text-[#0D1117] leading-tight">
                        {invite.staff_profile_id ? "Edit Staff Profile" : "Set Up Staff Profile"}
                    </h1>
            ...
            <div className="max-w-[720px] mx-auto px-4 py-6 space-y-3">
                ...
                <div className="bg-white rounded-2xl border border-black/[0.09] p-4 flex items-center gap-4 mb-2">
                    ...
                    <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                        <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-[#E8F2EB] text-[#2F5C3A]">Accepted</span>
                        {invite.staff_profile_id && (
                            <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-[#F7F4EF] text-[#8A9BB0]">Profile exists</span>
                        )}
                    </div>
                </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            <Field label="Profile Photo URL">
                                <div className="relative">
                                    <Camera size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#8A9BB0] pointer-events-none" />
                                    <input className={inputCls + " pl-8"} placeholder="https://…" value={form.profile_photo_url} onChange={set("profile_photo_url")} />
                                </div>
                            </Field>
                        </div>
                ...
                <div id="section-contact">
                    <SectionCard title="Contact" icon={Phone} active={activeSection === "contact"} completed={completedSections.has("contact")} onClick={() => setActiveSection("contact")}>
                        <div className="grid grid-cols-2 gap-3">
                            <Field label="Phone">
                                <input className={inputCls} placeholder="08012345678" value={form.phone} onChange={set("phone")} />
                            </Field>
                            <Field label="Alt. Phone">
                                <input className={inputCls} placeholder="08012345678" value={form.alt_phone} onChange={set("alt_phone")} />
                            </Field>
                            <Field label="City">
                                <input className={inputCls} placeholder="Lagos" value={form.city} onChange={set("city")} />
                            </Field>
                            <Field label="State">
                                <input className={inputCls} placeholder="State" value={form.state_id} onChange={set("state_id")} />
                            </Field>
                    ...
                    <SectionCard title="Employment" icon={Briefcase} active={activeSection === "employment"} completed={completedSections.has("employment")} onClick={() => setActiveSection("employment")}>
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            <Field label="Date Joined">
                                <input type="date" className={inputCls} value={form.date_joined} onChange={set("date_joined")} />
                            </Field>
                            <Field label="Salary">
                                <div className="relative">
                    ...
                    <SectionCard title="Professional" icon={HeartPulse} active={activeSection === "medical"} completed={completedSections.has("medical")} onClick={() => setActiveSection("medical")}>
                        <div className="grid grid-cols-2 gap-3">
                            <Field label="Specialization">
                                <input className={inputCls} placeholder="e.g. Cardiology" value={form.specialization} onChange={set("specialization")} />
                            </Field>
                            <Field label="Qualification">
                                <input className={inputCls} placeholder="e.g. MBBS, RN" value={form.qualification} onChange={set("qualification")} />
                            </Field>
                            <Field label="License Number">
                                <input className={inputCls} placeholder="MDC-123456" value={form.license_number} onChange={set("license_number")} />
                            </Field>
                            <Field label="License Expiry">
                                <input type="date" className={inputCls} value={form.license_expiry} onChange={set("license_expiry")} />
                            </Field>
                        </div>
                        <div className="mt-4 flex justify-between">
                    ...
                    <SectionCard title="Emergency Contact" icon={ShieldCheck} active={activeSection === "emergency"} completed={completedSections.has("emergency")} onClick={() => setActiveSection("emergency")}>
                        <div className="grid grid-cols-2 gap-3">
                            <Field label="Contact Name">
                                <input className={inputCls} placeholder="Full name" value={form.emergency_contact_name} onChange={set("emergency_contact_name")} />
                            </Field>
                            <Field label="Relationship">
                                <input className={inputCls} placeholder="e.g. Spouse, Parent" value={form.emergency_contact_relationship} onChange={set("emergency_contact_relationship")} />
                            </Field>
                            <Field label="Contact Phone" span={2}>
                                <input className={inputCls} placeholder="08012345678" value={form.emergency_contact_phone} onChange={set("emergency_contact_phone")} />
                            </Field>
                        </div>
                        <div className="mt-4 flex justify-between">
